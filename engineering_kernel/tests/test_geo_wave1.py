...
class GeoWave1Tests(unittest.TestCase):
    ...
    def test_layer(self):
        self.assertEqual(layer_thickness(self.layer), 2.0)
    ...
    def test_stresses(self):
        ...
        self.assertEqual(total_vertical_stress(18.0, 2.0), 36.0)
        self.assertEqual(effective_vertical_stress(36.0, 10.0), 26.0)
        self.assertEqual(stress_increment_uniform_load(100.0, 0.5), 50.0)
    ...
    def test_lateral_pressure(self):
        ...
        self.assertEqual(triangular_pressure_resultant(10.0, 3.0), 15.0)
        self.assertEqual(triangular_pressure_application_height(3.0), 1.0)

    def test_bearing_factors(self):
        self.assertEqual(bearing_capacity_factor_nq(0.0), 1.0)
        self.assertEqual(bearing_capacity_factor_nc(0.0), 5.14)
        self.assertEqual(bearing_capacity_factor_ngamma(0.0), 0.0)

    def test_bearing_capacity(self):
        qult = ultimate_bearing_capacity_strip(0.0, 20.0, 18.0, 1.0, 0.0)
        self.assertEqual(qult, 20.0)
        self.assertEqual(allowable_bearing_capacity(300.0, 3.0), 100.0)

    def test_settlement(self):
        self.assertEqual(one_dimensional_settlement(100.0, 2.0, 10000.0), 0.02)
        value = elastic_settlement(100.0, 1.0, 10000.0, 0.3)
        self.assertAlmostEqual(value, 0.0091)

    def test_consolidation(self):
        self.assertEqual(consolidation_degree_time_factor(0.0), 0.0)
        self.assertGreater(consolidation_degree_time_factor(0.1), 0.0)

    def test_seepage(self):
        self.assertEqual(hydraulic_gradient(2.0, 4.0), 0.5)
        self.assertEqual(darcy_velocity(1e-5, 0.5), 5e-6)
        self.assertEqual(seepage_discharge(1e-5, 0.5, 10.0), 5e-5)
        self.assertAlmostEqual(critical_hydraulic_gradient(20.0), 10.19/9.81)

    def test_liquefaction(self):
        self.assertEqual(liquefaction_safety_factor(0.2, 0.1), 2.0)

    def test_slope(self):
        self.assertEqual(slope_ratio(2.0, 1.0), 2.0)
        self.assertAlmostEqual(slope_angle_deg(1.0, 1.0), 45.0)

    def test_groundwater(self):
        self.assertEqual(groundwater_elevation(0.0, 0.5), -0.5)
