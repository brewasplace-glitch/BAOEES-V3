...
BACKUP_PATH = Path("backups/core_main_before_structural_reinforcement_engine_connect_v4.py")

IMPORT_LINE = "from baoees.structural_reinforcement_engine.main import StructuralReinforcementEngine\n"
LABEL = "Structural Element Sizing Engine resultaat:"
...
def find_statement_start(lines, label_index):
    for index in range(label_index, max(label_index - 10, -1), -1):
...
def add_reinforcement_block(text):
    if "structural_reinforcement_result" in text:
        print("Structural Reinforcement Engine lijkt al gekoppeld. Blok wordt niet opnieuw toegevoegd.")
        return text

    lines = text.splitlines(keepends=True)
    insert_index = find_insert_index(lines)
    lines.insert(insert_index, REINFORCEMENT_BLOCK)
...
def connect_test():
    ...
    try:
        ...
        text = add_reinforcement_block(text)
        write_core(text)

        print("STRUCTURAL_REINFORCEMENT_ENGINE_GEKKOPPELD_AAN_CORE")
...
def commit():
    ...
    run_command("git add tools_connect_structural_reinforcement_engine_to_core_v4.py")
    run_command('git commit -m "feat: connect Structural Reinforcement Engine to BAOEES Core"')
