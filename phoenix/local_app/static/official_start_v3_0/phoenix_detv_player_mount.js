...
/* PHOENIX DE TV SINGLE VISUAL AUTHORITY PARENT BRIDGE v1.0
   The local sidecar iframe is the sole PAT-002 visual presentation authority.
   Existing outer DE TV controls are preserved and bridged to the iframe. */
;(function phoenixSingleVisualAuthorityParentBridge(){
  "use strict";
  ...
  function intercept(ev){
    ...
    const action=controlAction(b);
    if(!action) return;
    if(!document.getElementById("phoenixOpenSourceMediaPlayer")) return;
  ...
  function interceptKey(ev){
    const input=document.getElementById("phoenixTvCommand");
    if(!input || ev.target!==input || ev.key!=="Enter") return;
    if(!document.getElementById("phoenixOpenSourceMediaPlayer")) return;
  ...
  function enforce(){
    ...
    if(f){
      ...
      f.style.width="100%";
      f.style.height="100%";
      status("OPEN-SOURCE PLAYER");
      meta("DE TV Â· SINGLE VISUAL AUTHORITY Â· lokale sidecar");
    }
  }
  ...
  window.addEventListener("message",ev=>{
    ...
    if(ev.data.type==="phoenix-detv-player-ready"){
      ...
      removeLegacyOverlays();
      if(ev.data.label) meta(String(ev.data.label));
      if(ev.data.state) status(String(ev.data.state));
    }
  });
  ...
  const begin=()=>{
    enforce();
    const panel=document.getElementById("phoenixTvPanel");
    if(panel) observer.observe(panel,{childList:true,subtree:true,attributes:true,attributeFilter:["style","class"]});
  ...
  window.PHOENIX_DE_TV_SINGLE_VISUAL_AUTHORITY_V1_0={
    send,enforce,removeLegacyOverlays
  };
})();