...
import json
import math
from phoenix.autonomy.european_certified_supply_priority import annotate_candidate, import_sort_key, ready_mix_import_allowed
# PHOENIX_EUROPEAN_CERTIFIED_SUPPLY_PRIORITY_v1_0
...
def build_global_material_sourcing_context(
    *, repository: Path, workspace: Path, project_id: str,
    ...
    # PHOENIX_GLOBAL_SUPPLIER_IMPORT_ACQUISITION_HOOK_v1_0
    acquisition_summary={"status":"NOT_RUN","blockers":[]}
    try:
        from phoenix.autonomy.global_supplier_import_acquisition import acquire_global_supplier_import_evidence
        _a=acquire_global_supplier_import_evidence(repository=repository,workspace=workspace,project_id=project_id,project_context=project_context,local_selection_register=local_selection_register,manifest=manifest)
        acquisition_summary={"status":_a.status,"register":_a.register,"request_register":_a.request_register,"written_catalogs":_a.written_catalogs,"blockers":_a.blockers}
    except Exception as exc:
        acquisition_summary={"status":"BLOCKED","blockers":[{"reason":"GLOBAL_IMPORT_ACQUISITION_ENGINE_ERROR","message":str(exc)}]}
    ...
    for req in requirements:
        ...
        for c in rows:
            ...
            importability_ok = local or ready_mix_import_allowed(c, family)
            valid = availability_ok and cert_ok and eng_ok and freshness_ok and landed.get("status") == "PASSED" and importability_ok
            evaluated.append(annotate_candidate({"product_id": c.get("product_id"), "supplier_name": c.get("supplier_name"), "material_family": family, "origin_country_code": origin or None, "supply_origin": "LOCAL" if local else "IMPORT", "availability_ok": availability_ok, "certification_ok": cert_ok, "engineering_qualification_ok": eng_ok, "freshness_ok": freshness_ok, "freshness_reasons": freshness_reasons, "technical_reasons": eng_reasons, "certification_reasons": cert_reasons, "importability_ok": importability_ok, "landed_cost": landed, "lead_time_days": c.get("lead_time_days"), "source_reference": c.get("source_reference"), "valid_for_selection": valid, "candidate": c}))
        valid = [x for x in evaluated if x["valid_for_selection"]]
        valid.sort(key=import_sort_key)
        if valid:
