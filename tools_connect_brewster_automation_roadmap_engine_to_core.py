...
BACKUP_PATH = Path("backups/core_main_before_brewster_automation_roadmap_engine_connect.py")

IMPORT_LINE = "from baoees.brewster_automation_roadmap_engine.main import BrewsterAutomationRoadmapEngine\n"
...
def find_statement_start(lines, label_index):
    for index in range(label_index, max(label_index - 18, -1), -1):
...
def add_roadmap_block(text):
    if "brewster_automation_roadmap_result" in text:
        print("Brewster Automation Roadmap Engine lijkt al gekoppeld. Blok wordt niet opnieuw toegevoegd.")
        return text
    ...
    lines.insert(insert_index, ROADMAP_BLOCK)
...
def inspect():
    ...
    print("Zoeken naar Brewster Automation Roadmap koppelpunt...")
...
def connect_test():
    ...
    try:
        ...
        text = add_roadmap_block(text)
        write_core(text)

        print("BREWSTER_AUTOMATION_ROADMAP_ENGINE_GEKKOPPELD_AAN_CORE")
...
def commit():
    ...
    run_command("git add tools_connect_brewster_automation_roadmap_engine_to_core.py")
    run_command('git commit -m "feat: connect Brewster Automation Roadmap Engine to BAOEES Core"')
