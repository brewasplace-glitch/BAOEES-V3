...
ENGINE_DIR = Path("baoees/structural_load_engine")
INIT_PATH = ENGINE_DIR / "__init__.py"
MAIN_PATH = ENGINE_DIR / "main.py"


INIT_CONTENT = """from .main import StructuralLoadEngine
"""
...
def run_basic_tests():
    ...
    module = importlib.import_module("baoees.structural_load_engine.main")
    engine_class = getattr(module, "StructuralLoadEngine")

    engine = engine_class()

    result = engine.create_structural_load_analysis(
        project_result={
            "project_id": "test",
    ...
    if result.get("status") != "STRUCTURAL_LOAD_ANALYSIS_GEREED":
        raise RuntimeError("Structural Load Engine gaf geen correcte status terug.")

    if result.get("imposed_loads", {}).get("main_imposed_load_kN_m2") != 1.75:
        raise RuntimeError("Veranderlijke belasting voor woonfunctie is niet 1.75.")
...
def main():
    ...
    print("STRUCTURAL_LOAD_ENGINE_V1_AANGEMAAKT")
    print(f"Init: {INIT_PATH}")
    print(f"Main: {MAIN_PATH}")
    print(f"Status: {result.get('status')}")
    print(f"Woonfunctie veranderlijke belasting: {result.get('imposed_loads', {}).get('main_imposed_load_kN_m2')}")
