# PROJECT PHOENIX v7.8 UPDATE
# Phoenix Automated Task Builder
# Geen handmatig Python knip- en plakwerk nodig.

$ErrorActionPreference = "Stop"

Write-Host "PROJECT PHOENIX v7.8 UPDATE START" -ForegroundColor Cyan
...
$EnginePath = Join-Path $ProjectRoot "apps\brewster_engineering_wizard\project_analyzer\phoenix_automated_task_builder_engine.py"
$ScriptDir = Join-Path $ProjectRoot "scripts"
$RunnerPs1 = Join-Path $ScriptDir "START_PROJECTANALYSE_v7_8.ps1"
$RunnerBat = Join-Path $ScriptDir "START_PROJECTANALYSE_v7_8.bat"
$LogPath = Join-Path $ProjectRoot "outputs\projects\start_projectanalyse_v7_8_update_log.json"
...
foreach ($Path in @($EnginePath, $RunnerPs1, $RunnerBat)) {
    if (Test-Path $Path) {
        Copy-Item $Path "$Path.backup_v7_8_$Timestamp" -Force
    }
}
...
$UpdateLog = [ordered]@{
    status = "OPGESLAGEN"
    engine = "Project Phoenix Automated Task Builder Connector"
    engine_version = "v7.8"
...
Write-Host "Syntaxcontrole Phoenix Automated Task Builder Engine..." -ForegroundColor Cyan
python -m py_compile .\apps\brewster_engineering_wizard\project_analyzer\phoenix_automated_task_builder_engine.py

Write-Host "Run v7.8..." -ForegroundColor Cyan
powershell -ExecutionPolicy Bypass -File .\scripts\START_PROJECTANALYSE_v7_8.ps1

Write-Host "Git status..." -ForegroundColor Cyan
git status

Write-Host "PROJECT PHOENIX v7.8 UPDATE KLAAR" -ForegroundColor Green
