"""Generate real drawings and reports from a selectable central model."""
...
def main(argv=None)->int:
    p=argparse.ArgumentParser();p.add_argument('--output-dir',type=Path);p.add_argument('--verify-against-artifacts',type=Path);p.add_argument('--expect-production-ready',action='store_true');p.add_argument('--model-file',type=Path);args=p.parse_args(argv)
    temp=None
    if args.output_dir is None:temp=tempfile.TemporaryDirectory();out=Path(temp.name)
    else:out=args.output_dir
    model_path=(args.model_file.resolve() if args.model_file else ROOT/MODEL)
    base=json.loads((ROOT/BASE).read_text(encoding='utf-8'));model=json.loads(model_path.read_text(encoding='utf-8'));config=derive_production_config(base,model)
