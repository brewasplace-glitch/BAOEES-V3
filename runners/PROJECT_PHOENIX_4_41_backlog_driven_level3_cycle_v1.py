...
from phoenix.autonomy import BacklogDrivenLevel3CycleService
...
def synthetic_repository(policy_root: Path, root: Path) -> tuple[Path, str, Path]:
    ...
    git(repo, "add", ".")
    git(repo, "commit", "-m", "phase12 synthetic package baseline")
...
def main() -> int:
    ...
    mode.add_argument("--live-cycle", action="store_true")
    ...
    runtime = Path(args.runtime_root).resolve() if args.runtime_root else (
        Path.home() / ".project-phoenix" / "autonomy" / "phase12"
    )
    if args.probe:
        service = BacklogDrivenLevel3CycleService(
            repo, runtime, policy_root=policy_root, enable_gateway=False
        )
        result = service.probe()
    elif args.synthetic_proof:
        with tempfile.TemporaryDirectory(prefix="phoenix-phase12-proof-") as temporary:
            proof_root = Path(temporary)
            fixture, baseline, receipt = synthetic_repository(policy_root, proof_root)
            service = BacklogDrivenLevel3CycleService(
                fixture,
                runtime,
