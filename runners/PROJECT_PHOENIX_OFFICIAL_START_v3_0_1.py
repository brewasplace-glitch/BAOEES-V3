#!/usr/bin/env python3
"""Launcher/runtime for Phoenix Official Start Screen v3.0.1."""
...
def find_live_runtime():
    ...
    if not value:
        return None
    if value.get("start_screen_version") != "3.0.1":
        return None
    return port
...
def refresh_autosync():
    refresh = (
        ...
        / "tools"
        / "start_screen"
        / "REFRESH_PROJECT_PHOENIX_OFFICIAL_START_v3.ps1"
    )
...
def launch():
    ...
    if port is None:
        ...
        while time.time() < deadline:
            ...
            time.sleep(0.25)
    if port is None:
        raise RuntimeError("Phoenix 3.0.1 local runtime kon niet worden gestart.")
    url = f"http://127.0.0.1:{port}/start-v3/"
    webbrowser.open(url)
    print(f"PROJECT PHOENIX OFFICIAL START v3.0.1: OPENED -> {url}")
