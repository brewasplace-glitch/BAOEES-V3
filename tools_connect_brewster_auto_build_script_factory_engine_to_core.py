...
BACKUP_PATH = Path("backups/core_main_before_brewster_auto_build_script_factory_engine_connect.py")

IMPORT_LINE = "from baoees.brewster_auto_build_script_factory_engine.main import BrewsterAutoBuildScriptFactoryEngine\n"

PREFERRED_LABELS = [
    "Brewster Task Orchestrator Engine resultaat:",
    "Brewster Automation Roadmap Engine resultaat:",
    "Phoenix UI Mapping Engine resultaat:",
...
def find_statement_start(lines, label_index):
    for index in range(label_index, max(label_index - 20, -1), -1):
...
def add_factory_block(text):
    if "brewster_auto_build_script_factory_result" in text:
        print("Brewster Auto Build Script Factory Engine lijkt al gekoppeld. Blok wordt niet opnieuw toegevoegd.")
        return text
    ...
    lines.insert(insert_index, FACTORY_BLOCK)
...
def inspect():
    ...
    print("Zoeken naar Brewster Auto Build Script Factory koppelpunt...")
...
def connect_test():
    ...
    try:
        ...
        text = add_factory_block(text)
        write_core(text)

        print("BREWSTER_AUTO_BUILD_SCRIPT_FACTORY_ENGINE_GEKKOPPELD_AAN_CORE")
...
def commit():
    ...
    run_command("git add tools_connect_brewster_auto_build_script_factory_engine_to_core.py")
    run_command('git commit -m "feat: connect Brewster Auto Build Script Factory Engine to BAOEES Core"')
