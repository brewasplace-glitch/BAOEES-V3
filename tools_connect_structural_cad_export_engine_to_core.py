...
BACKUP_PATH = Path("backups/core_main_before_structural_cad_export_engine_connect.py")

IMPORT_LINE = "from baoees.structural_cad_export_engine.main import StructuralCADExportEngine\n"
LABEL = "Structural Drawing Package Engine resultaat:"
...
def add_cad_export_block(text):
    if "structural_cad_export_result" in text:
        print("Structural CAD Export Engine lijkt al gekoppeld. Blok wordt niet opnieuw toegevoegd.")
        return text

    lines = text.splitlines(keepends=True)
    insert_index = find_insert_index(lines)
    lines.insert(insert_index, CAD_EXPORT_BLOCK)
...
def connect_test():
    ...
    try:
        ...
        text = add_cad_export_block(text)
        write_core(text)

        print("STRUCTURAL_CAD_EXPORT_ENGINE_GEKKOPPELD_AAN_CORE")
...
def commit():
    ...
    run_command("git add tools_connect_structural_cad_export_engine_to_core.py")
    run_command('git commit -m "feat: connect Structural CAD Export Engine to BAOEES Core"')
