...
BACKUP_PATH = Path("backups/core_main_before_structural_drawing_package_engine_connect.py")

IMPORT_LINE = "from baoees.structural_drawing_package_engine.main import StructuralDrawingPackageEngine\n"
LABEL = "Structural Calculation Report Engine resultaat:"


DRAWING_PACKAGE_BLOCK = '''
        try:
...
def add_drawing_package_block(text):
    if "structural_drawing_package_result" in text:
        print("Structural Drawing Package Engine lijkt al gekoppeld. Blok wordt niet opnieuw toegevoegd.")
        return text

    lines = text.splitlines(keepends=True)
    insert_index = find_insert_index(lines)
    lines.insert(insert_index, DRAWING_PACKAGE_BLOCK)
...
def connect_test():
    ...
    try:
        ...
        text = add_drawing_package_block(text)
        write_core(text)

        print("STRUCTURAL_DRAWING_PACKAGE_ENGINE_GEKKOPPELD_AAN_CORE")
...
def commit():
    ...
    run_command("git add tools_connect_structural_drawing_package_engine_to_core.py")
    run_command('git commit -m "feat: connect Structural Drawing Package Engine to BAOEES Core"')
