...
BACKUP_PATH = Path("backups/core_main_before_brewster_task_orchestrator_engine_connect.py")

IMPORT_LINE = "from baoees.brewster_task_orchestrator_engine.main import BrewsterTaskOrchestratorEngine\n"
...
def add_orchestrator_block(text):
    if "brewster_task_orchestrator_result" in text:
        print("Brewster Task Orchestrator Engine lijkt al gekoppeld. Blok wordt niet opnieuw toegevoegd.")
        return text
    ...
    lines.insert(insert_index, ORCHESTRATOR_BLOCK)
...
def inspect():
    ...
    print("Zoeken naar Brewster Task Orchestrator koppelpunt...")
...
def connect_test():
    ...
    try:
        ...
        text = add_orchestrator_block(text)
        write_core(text)

        print("BREWSTER_TASK_ORCHESTRATOR_ENGINE_GEKKOPPELD_AAN_CORE")
...
def commit():
    ...
    run_command("git add tools_connect_brewster_task_orchestrator_engine_to_core.py")
    run_command('git commit -m "feat: connect Brewster Task Orchestrator Engine to BAOEES Core"')
