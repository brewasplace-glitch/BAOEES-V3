# PROJECT PHOENIX v6.6 UPDATE
# Doel: Brewster Engineering Wizard kennis migreren naar Project Phoenix BIB.
# Geen handmatig Python knip- en plakwerk nodig.

$ErrorActionPreference = "Stop"

Write-Host "PROJECT PHOENIX v6.6 UPDATE START" -ForegroundColor Cyan
...
$EnginePath = Join-Path $ProjectRoot "baoees\project_analyzer\brewster_knowledge_migration_engine.py"
$BatPath = Join-Path $ProjectRoot "START_PROJECTANALYSE.bat"
$Ps1Path = Join-Path $ProjectRoot "START_PROJECTANALYSE.ps1"
$VersionedBatPath = Join-Path $ProjectRoot "START_PROJECTANALYSE_v6_6.bat"
$VersionedPs1Path = Join-Path $ProjectRoot "START_PROJECTANALYSE_v6_6.ps1"
$LogPath = Join-Path $ProjectRoot "outputs\projects\start_projectanalyse_v6_6_update_log.json"

New-Item -ItemType Directory -Path (Split-Path $EnginePath) -Force | Out-Null
New-Item -ItemType Directory -Path (Split-Path $LogPath) -Force | Out-Null

if (Test-Path $EnginePath) {
    Copy-Item $EnginePath "$EnginePath.backup_v6_6_$Timestamp" -Force
}

if (Test-Path $BatPath) {
    Copy-Item $BatPath "$BatPath.backup_v6_6_$Timestamp" -Force
}

if (Test-Path $Ps1Path) {
    Copy-Item $Ps1Path "$Ps1Path.backup_v6_6_$Timestamp" -Force
}
...
$UpdateLog = [ordered]@{
    status = "OPGESLAGEN"
    engine = "Project Phoenix Brewster Knowledge Migration Connector"
    engine_version = "v6.6"
    generated_at = (Get-Date).ToString("s")
    project_root = "$ProjectRoot"
    migration_engine = "$EnginePath"
    ...
    purpose = "Migreert opgebouwde Brewster Engineering Wizard kennis naar de lokale Project Phoenix BIB en koppelt dit aan START_PROJECTANALYSE."
}
...
Write-Host " - baoees\project_analyzer\brewster_knowledge_migration_engine.py"
Write-Host " - START_PROJECTANALYSE.bat"
Write-Host " - START_PROJECTANALYSE.ps1"
Write-Host " - START_PROJECTANALYSE_v6_6.bat"
Write-Host " - START_PROJECTANALYSE_v6_6.ps1"
Write-Host " - outputs\projects\start_projectanalyse_v6_6_update_log.json"

Write-Host ""
Write-Host "Syntaxcontrole knowledge migration engine..." -ForegroundColor Cyan
python -m py_compile baoees\project_analyzer\brewster_knowledge_migration_engine.py

Write-Host ""
Write-Host "Test START_PROJECTANALYSE_v6_6.ps1..." -ForegroundColor Cyan
powershell -ExecutionPolicy Bypass -File .\START_PROJECTANALYSE_v6_6.ps1
...
Write-Host ""
Write-Host "PROJECT PHOENIX v6.6 UPDATE KLAAR" -ForegroundColor Green
