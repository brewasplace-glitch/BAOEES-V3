...
ENGINE_DIR = Path("baoees/structural_reinforcement_engine")
INIT_PATH = ENGINE_DIR / "__init__.py"
MAIN_PATH = ENGINE_DIR / "main.py"

INIT_CONTENT = "from .main import StructuralReinforcementEngine\n"
...
def create_test():
    ...
    module = importlib.import_module("baoees.structural_reinforcement_engine.main")
    engine_class = getattr(module, "StructuralReinforcementEngine")
    engine = engine_class()

    result = engine.create_structural_reinforcement_design(
        project_result={
            "project_id": "test",
            "project_name": "Testproject"
        },
        structural_sizing_result={
            "slab_sizing": {"recommended_slab_thickness_mm": 200},
            "beam_sizing": {
                "recommended_beam_width_mm": 250,
                "recommended_beam_height_mm": 500
            },
            "column_sizing": {
                "recommended_column_width_mm": 300,
                "recommended_column_depth_mm": 300
            },
            "wall_sizing": {"recommended_wall_thickness_mm": 200}
        },
        foundation_design_result={
            "strip_foundation": {
                "recommended_width_mm": 1500,
                "recommended_height_mm": 400
            },
            "foundation_beam": {
                "recommended_width_mm": 500,
                "recommended_height_mm": 600
            }
        },
        foundation_verification_result={
            "verification_summary": {"status": "INDICATIEF_OK"}
        },
        element_load_result={
            "floor_loads": [{"floor_number": 1, "uls_floor_load_kN_m2": 14.0}],
            "foundation_line_loads": {"combined_foundation_line_load_kN_m": 90.0}
        }
    )

    if result.get("status") != "STRUCTURAL_REINFORCEMENT_DESIGN_GEREED":
        raise RuntimeError("Structural Reinforcement Engine gaf geen correcte status terug.")

    if not result.get("slab_reinforcement", {}).get("bottom_reinforcement"):
        raise RuntimeError("Geen vloerwapening gegenereerd.")

    if not result.get("foundation_beam_reinforcement", {}).get("bottom_bars"):
        raise RuntimeError("Geen funderingsbalkwapening gegenereerd.")

    print("")
    print("STRUCTURAL_REINFORCEMENT_ENGINE_TEST_OK")
    print(f"Status: {result.get('status')}")
    print(f"Vloerwapening: {result.get('slab_reinforcement', {}).get('bottom_reinforcement')}")
    print(
        "Funderingsbalkwapening: "
        f"{result.get('foundation_beam_reinforcement', {}).get('bottom_bars')}"
    )
    print("")
    print("STRUCTURAL_REINFORCEMENT_ENGINE_V1_AANGEMAAKT")
...
def commit():
    ...
    run_command("git add baoees/structural_reinforcement_engine/__init__.py")
    run_command("git add baoees/structural_reinforcement_engine/main.py")
    run_command("git add tools_create_structural_reinforcement_engine_v1.py")
    run_command('git commit -m "feat: add Structural Reinforcement Engine v1"')
