...
ENGINE_DIR = Path("baoees/structural_element_sizing_engine")
INIT_PATH = ENGINE_DIR / "__init__.py"
MAIN_PATH = ENGINE_DIR / "main.py"

INIT_CONTENT = "from .main import StructuralElementSizingEngine\n"

ELEMENT_ENGINE_CODE = r'''from datetime import datetime
...
def create_engine():
    ...
    MAIN_PATH.write_text(ELEMENT_ENGINE_CODE, encoding="utf-8")
    test_engine()
    print("")
    print("STRUCTURAL_ELEMENT_SIZING_ENGINE_V1_AANGEMAAKT")
...
def test_engine():
    ...
    module = importlib.import_module("baoees.structural_element_sizing_engine.main")
    engine_class = getattr(module, "StructuralElementSizingEngine")
    engine = engine_class()

    result = engine.create_structural_element_sizing(
        project_result={
            "project_id": "test",
        ...
        element_load_result={
            ...
            "floor_loads": [
                {
                    "uls_floor_load_kN_m2": 10.2
                }
            ],
            "column_loads": {
                "uls_load_per_column_kN": 510
            },
            "foundation_line_loads": {
                "combined_foundation_line_load_kN_m": 72
            }
        },
        foundation_design_result={
            "recommended_foundation": {
                "strip_width_mm": 1500,
                "foundation_beam_width_mm": 500,
                "foundation_beam_height_mm": 600,
                "status": "VOORLOPIG"
            }
        },
        foundation_verification_result={
    ...
    if result.get("status") != "STRUCTURAL_ELEMENT_SIZING_GEREED":
        raise RuntimeError("Structural Element Sizing Engine gaf geen correcte status terug.")

    if result.get("slab_design", {}).get("recommended_thickness_mm", 0) <= 0:
        raise RuntimeError("Geen vloerdikte gegenereerd.")

    if result.get("foundation_element_design", {}).get("recommended_strip_width_mm", 0) <= 0:
        raise RuntimeError("Geen funderingselement-dimensie gegenereerd.")

    print("")
    print("STRUCTURAL_ELEMENT_SIZING_ENGINE_TEST_OK")
    print(f"Status: {result.get('status')}")
    print(f"Vloerdikte: {result.get('slab_design', {}).get('recommended_thickness_mm')} mm")
    print(f"Balk: {result.get('sizing_summary', {}).get('recommended_beam_size_mm')} mm")
    print(f"Kolom: {result.get('sizing_summary', {}).get('recommended_column_size_mm')} mm")
    print(f"Funderingstrook: {result.get('sizing_summary', {}).get('recommended_foundation_strip_width_mm')} mm")
...
def commit_engine():
    ...
    run_command("git add baoees/structural_element_sizing_engine/__init__.py")
    run_command("git add baoees/structural_element_sizing_engine/main.py")
    run_command("git add tools_create_structural_element_sizing_engine_v1.py")
    run_command('git commit -m "feat: add Structural Element Sizing Engine v1"')
