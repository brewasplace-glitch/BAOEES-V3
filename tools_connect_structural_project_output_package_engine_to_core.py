...
BACKUP_PATH = Path("backups/core_main_before_structural_project_output_package_engine_connect.py")

IMPORT_LINE = "from baoees.structural_project_output_package_engine.main import StructuralProjectOutputPackageEngine\n"
...
OUTPUT_PACKAGE_BLOCK = '''
        try:
            structural_project_output_package_engine = StructuralProjectOutputPackageEngine()
            structural_project_output_package_result = structural_project_output_package_engine.create_structural_project_output_package(
            ...
            structural_project_output_package_result = {
                ...
                "status": "STRUCTURAL_PROJECT_OUTPUT_PACKAGE_ENGINE_ERROR",
                "error": str(error)
            }
...
def add_output_package_block(text):
    if "structural_project_output_package_result" in text:
        print("Structural Project Output Package Engine lijkt al gekoppeld. Blok wordt niet opnieuw toegevoegd.")
        return text
    ...
    lines.insert(insert_index, OUTPUT_PACKAGE_BLOCK)
...
def connect_test():
    ...
    try:
        ...
        text = add_output_package_block(text)
        write_core(text)

        print("STRUCTURAL_PROJECT_OUTPUT_PACKAGE_ENGINE_GEKKOPPELD_AAN_CORE")
...
def commit():
    ...
    run_command("git add tools_connect_structural_project_output_package_engine_to_core.py")
    run_command('git commit -m "feat: connect Structural Project Output Package Engine to BAOEES Core"')
