# PROJECT PHOENIX v7.7 UPDATE
# Phoenix Task Executor / Build Script Factory
# Geen handmatig Python knip- en plakwerk nodig.

$ErrorActionPreference = "Stop"

Write-Host "PROJECT PHOENIX v7.7 UPDATE START" -ForegroundColor Cyan
...
$EnginePath = Join-Path $ProjectRoot "apps\brewster_engineering_wizard\project_analyzer\phoenix_task_executor_engine.py"
$ScriptDir = Join-Path $ProjectRoot "scripts"
$RunnerPs1 = Join-Path $ScriptDir "START_PROJECTANALYSE_v7_7.ps1"
$RunnerBat = Join-Path $ScriptDir "START_PROJECTANALYSE_v7_7.bat"
$LogPath = Join-Path $ProjectRoot "outputs\projects\start_projectanalyse_v7_7_update_log.json"
...
foreach ($Path in @($EnginePath, $RunnerPs1, $RunnerBat)) {
    if (Test-Path $Path) {
        Copy-Item $Path "$Path.backup_v7_7_$Timestamp" -Force
    }
}
...
$UpdateLog = [ordered]@{
    status = "OPGESLAGEN"
    engine = "Project Phoenix Task Executor Connector"
    engine_version = "v7.7"
    generated_at = (Get-Date).ToString("s")
...
Write-Host "Syntaxcontrole Phoenix Task Executor Engine..." -ForegroundColor Cyan
python -m py_compile .\apps\brewster_engineering_wizard\project_analyzer\phoenix_task_executor_engine.py

Write-Host "Run v7.7..." -ForegroundColor Cyan
powershell -ExecutionPolicy Bypass -File .\scripts\START_PROJECTANALYSE_v7_7.ps1

Write-Host "Git status..." -ForegroundColor Cyan
git status

Write-Host "PROJECT PHOENIX v7.7 UPDATE KLAAR" -ForegroundColor Green
