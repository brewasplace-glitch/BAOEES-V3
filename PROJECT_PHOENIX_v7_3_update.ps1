# PROJECT PHOENIX v7.3 UPDATE
# Foundation Engine
# Geen handmatig Python knip- en plakwerk nodig.

$ErrorActionPreference = "Stop"

Write-Host "PROJECT PHOENIX v7.3 UPDATE START" -ForegroundColor Cyan
...
$EnginePath = Join-Path $ProjectRoot "baoees\project_analyzer\foundation_engine.py"
$BatPath = Join-Path $ProjectRoot "START_PROJECTANALYSE.bat"
$Ps1Path = Join-Path $ProjectRoot "START_PROJECTANALYSE.ps1"
$VersionedBatPath = Join-Path $ProjectRoot "START_PROJECTANALYSE_v7_3.bat"
$VersionedPs1Path = Join-Path $ProjectRoot "START_PROJECTANALYSE_v7_3.ps1"
$LogPath = Join-Path $ProjectRoot "outputs\projects\start_projectanalyse_v7_3_update_log.json"
...
foreach ($Path in @($EnginePath, $BatPath, $Ps1Path)) {
    if (Test-Path $Path) {
        Copy-Item $Path "$Path.backup_v7_3_$Timestamp" -Force
    }
}
...
echo PROJECT PHOENIX / BAOEES - START PROJECTANALYSE v7.3
...
echo PROJECT PHOENIX v7.3 START PROJECTANALYSE KLAAR

if exist "outputs\projects\project_foundation_dashboard_v7_3.html" (
    start "" "outputs\projects\project_foundation_dashboard_v7_3.html"
)
...
:error
echo FOUT: START PROJECTANALYSE v7.3 is gestopt.
git status
pause
exit /b 1
'@
...
Write-Host "PROJECT PHOENIX / BAOEES - START PROJECTANALYSE v7.3" -ForegroundColor Cyan
...
Write-Host "PROJECT PHOENIX v7.3 START PROJECTANALYSE KLAAR" -ForegroundColor Green

$Dashboard = Join-Path $PSScriptRoot "outputs\projects\project_foundation_dashboard_v7_3.html"
...
$UpdateLog = [ordered]@{
    status = "OPGESLAGEN"
    engine = "Project Phoenix Foundation Connector"
    engine_version = "v7.3"
    generated_at = (Get-Date).ToString("s")
    project_root = "$ProjectRoot"
    foundation_engine = "$EnginePath"
    purpose = "Voegt funderingsconcept, varianten, controles en output voor constructie-engine toe aan START_PROJECTANALYSE."
}
...
Write-Host "Syntaxcontrole foundation engine..." -ForegroundColor Cyan
python -m py_compile baoees\project_analyzer\foundation_engine.py

Write-Host "Test START_PROJECTANALYSE_v7_3.ps1..." -ForegroundColor Cyan
powershell -ExecutionPolicy Bypass -File .\START_PROJECTANALYSE_v7_3.ps1

Write-Host "Git status..." -ForegroundColor Cyan
git status

Write-Host "PROJECT PHOENIX v7.3 UPDATE KLAAR" -ForegroundColor Green
